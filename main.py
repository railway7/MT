...
###获取pushPlusToken
def getpushPlusToken(s):
    global yesornot2
    global pushPlusToken
    yesornot2 = "y"
    pushPlusToken = requests.get(url_ip + "MT/get_users/spiritlhl").json()[s]["pushPlusToken"]
    return -1


# 获取token
def gettoken(s):
    token = requests.get(url_ip+"MT/get_users/spiritlhl").json()[s]["token"]
    return token


# 获取经纬度函数并存入当前目录文本(美团活动为随机地点固定半年以上,各地大额红包概率可能不同，若长期小额，可尝试换地址或换号)
def getlatlongitude(s):
    latitude = requests.get(url_ip + "MT/get_users/spiritlhl").json()[s]["wm_latitude"]
    return latitude
    ...
    try:
        ...
        if (result2["code"] == 0 and result2["subcode"] == 0 and len(result2["data"]["propExchangeRuleInfos"])):
            for k in result2["data"]["propExchangeRuleInfos"]:
                print(
                    "第%d类必中符 所需设置propId参数为%d\t所需红包豆数量为:%d\t总量为%d\n" % (cent, k["propId"], k["needNumber"], k["amount"]))
                cent = cent + 1
            print(
                "一般这几类必中符金额依次为5元 8元 15元,大概率使用后兑换到20-5，25-8,40-15的红包，建议选择面值最大的一类,即propId填5,所需豆子数量填1800即可\n脚本会自动从设定的面值去尝试兑换，逐级尝试面值，直到兑换成功，所以推荐设置默认兑换15面值的必中符\n注意填写的propId和所需豆子数之间是上方的一一对应关系，错误对应将导致兑换失败!\n")
...
def getpropId_Coinnumber(token, s):
    myredbean(token)
    propId = requests.get(url_ip + "MT/get_users/spiritlhl").json()[s]["propId"]
    exchangeCoinNumber = requests.get(url_ip + "MT/get_users/spiritlhl").json()[s]["exchangeCoinNumber"]
    return -1


# 定义从文本文件中获取存入变量的函数,第二次运行时不用输入，若需改变经纬度和token，则直接删除文件即可
def getVar(s):
    wm_latitude = requests.get(url_ip + "MT/get_users/spiritlhl").json()[s]["wm_latitude"]
    wm_longitude = requests.get(url_ip + "MT/get_users/spiritlhl").json()[s]["wm_longitude"]
    token = requests.get(url_ip + "MT/get_users/spiritlhl").json()[s]["token"]
    propId = requests.get(url_ip + "MT/get_users/spiritlhl").json()[s]["propId"]
    exchangeCoinNumber = requests.get(url_ip + "MT/get_users/spiritlhl").json()[s]["exchangeCoinNumber"]
...
# def 限时抢红包函数
def drawlottery(batchId, token, propIdforuse, s):
    wm_latitude = getVar(s)[0]
    wm_longitude = getVar(s)[1]
    print("**开始执行限时抢天天神券脚本🧧:**\n")
    datas = "parActivityId=" + parActivityId + "&wm_latitude=" + str(wm_latitude) + "&wm_longitude=" + str(
...
def pushPlus(s):
    global webhook
    pushPlusToken = pushPlusTokenvar(s)
...
def main(s):
    ...
    token = gettoken(s)
    getlatlongitude(s)
    getpropId_Coinnumber(token, s)
    sys.stdout = Logger(str(cwd) + r'/output.txt')
    print("脚本启动时间:%s\n" % (n_time))
    token = getVar(s)[2]
    signForBeans(token)
    queryredpool(token, s)
    batchId = getbatchId(token, s)
    if expire == 0:
        # print (yesornot)
        # print (yesornot2)
        if yesornot == "y":
            serverjiangforexpire(s)
        if yesornot2 == "y":
            pushPlusforexpire(s)
        sys.exit(0)
    if expire == 2:
...
def input(
                    name: str = Form(...),
                    wm_latitude: int = Form(...),
                    wm_longitude: int = Form(...),
                    token: str  = Form(...),
                    propId: int = Form(...),
                    exchangeCoinNumber: int = Form(...),
                    pushPlusToken: str = Form(...),
                    db: Session = Depends(get_db)
                ):
    db_user = curd.get_user_by_name(db, name=name)  # 查询是否在库
    data = {
        "name": name,
    ...
    if (db_user != None):
        curd.change_user_by_code(db=db, user=data)
        return "更新成功"
    else:
        curd.create_user_by_code(user=data, db=db)  # 不在库创建
        return "录入成功"

@application.get("/get_user/{id}", response_model=schemas.Readuser)
def get_user(id: int, db: Session = Depends(get_db)):
    db_user = curd.get_user(db, id=id)
    if db_user is None:
        raise HTTPException(status_code=404, detail="user not found")
    return db_user

@application.get("/get_user_by_name/{name}", response_model=schemas.Readuser)
def get_user(name: str, db: Session = Depends(get_db)):
    db_user = curd.get_user_by_name(db, name=name)
    if db_user is None:
        raise HTTPException(status_code=404, detail="user not found")
    return db_user
...
@application.get("/delete_user/{name}")
def delete_user(name: str, db: Session = Depends(get_db)): # 指定用户数据删除
    delete_user = curd.delete_user_by_code(db, name=name)
    return delete_user

@application.get(f"/main/{admin}")
def run_main(admin: str, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    if admin == admin:
        t = requests.get(url_ip + "MT/get_users/spiritlhl").json()
        for i in range(0,len(t)):
            background_tasks.add_task(main, i)
    return "共{}个号 开始执行任务".format(len(t))